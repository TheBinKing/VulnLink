...
class VulnLinkScanner:
    def __init__(self,custom_rules=None):
        ...
        self.functions_list = []
        if not custom_rules:
            with open(os.path.join(os.path.dirname(os.path.realpath(__file__)),"vulnlink_rules.json"),"r") as rules_file:
                self.rules = json.load(rules_file)
        else:
            self.rules = custom_rules
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)),"vulnlink_prototypes.json"),"r") as proto_file:
                ...
                for scanned_function_xref_tuple in xrefs_dict[scanned_function]:
                    ...
                    if params_raw is None:
                        if self.hexrays:
                            ...
                            params_raw = []
                    for p in params_raw:
                        param.append(VulnLinkScanner.Param(self,p,scanned_function_xref,scanned_function_name))
                    param_count = len(param)

                    function_call = VulnLinkScanner.FunctionCall(self,scanned_function_xref,scanned_function_name)
                    ...
                    try:
                        ...
                        priority = "Info"
                        #results.append(list(VulnLink.result_window_row(rule["name"],scanned_function_display_name,found_in_name,hex(scanned_function_xref),"Not Checked","Low","")))
                    except Exception:
                    ...
                    if priority and not "wrapped" in scanned_function_display_name:
                        results.append(list(VulnLink.result_window_row(rule["name"],scanned_function_display_name,found_in_name,hex(scanned_function_xref),"Not Checked",priority,"")))
...
class VulnLink_Single_Function(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        f = vulnlink_form_t(function_name)
        ...
        vulnlink_scanner = VulnLinkScanner(tmp_rule)
        rows = []
        marked_addrs = []
        vulnlink_data = {}
        # Load stored data
        ...
        for item in vulnlink_data:
            rows.append([vulnlink_data[item]["name"],vulnlink_data[item]["function"],vulnlink_data[item]["in"],vulnlink_data[item]["addr"],vulnlink_data[item]["status"],vulnlink_data[item]["priority"],vulnlink_data[item]["comment"]])
            marked_addrs.append(vulnlink_data[item]["addr"])

        # Run the scan for selected function
        print("[VulnLink] Started the scan ...")
        scan_result = vulnlink_scanner.start_scan(marked_addrs)
        ...
        # Save the results
        for item in rows:
            vulnlink_data[f"{item[3]}_{item[0]}"] = {"name":item[0],"function":item[1],"in":item[2],"addr":item[3],"status":item[4],"priority":item[5],"comment":item[6]}
...
class VulnLink(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        if vulnlink_data and answer == 1:
            for item in vulnlink_data:
                rows.append([vulnlink_data[item]["name"],vulnlink_data[item]["function"],vulnlink_data[item]["in"],vulnlink_data[item]["addr"],vulnlink_data[item]["status"],vulnlink_data[item]["priority"],vulnlink_data[item]["comment"]])
                marked_addrs.append(f'{vulnlink_data[item]["name"]}_{vulnlink_data[item]["addr"]}')
                ...
                if f.cType.value == 0:
                    ...
                    try:
                        with open(os.path.join(f.iFileOpen.value),"r") as rules_file:
                            vulnlink_scanner = VulnLinkScanner(json.load(rules_file))
                    except:
...
class Hooks(idaapi.UI_Hooks):
    ...
    def finish_populating_widget_popup(self, form, popup):
        action_text = f"Add '{utils.get_func_name(idc.here())}' function to VulnLink"
    ...
    def current_widget_changed(self, widget, prev_widget):
        ...
        if title and "VulnLink" in title and self.chooser:
